...
if __name__ == "__main__":
    ...
    (postcode_constituencies, unknowns) = map_postcodes_to_constituencies(
            "all_postcodes.csv",
            constituencies)
    postcode_constituencies.update(guess_unknown_postcodes(unknowns,
        constituencies))
    
    for postcode in postcode_constituencies:
        this_constituency = waw_app.models.Constituency.objects.filter(
                name__exact=postcode_constituencies[postcode])[0]
        new_postcode = waw_app.models.Postcode.objects.create(
                postcode=postcode, constituency=this_constituency)
        new_postcode.save()
